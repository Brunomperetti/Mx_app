...
if uploaded_file is not None:
    ...
    if ventas is not None:
        ...
        # --- MODO SEGMENTO ---
            ...
            segmento_promedio = st.sidebar.slider("Promedio Mensual de Compra", 0, int(ventas[columnas_meses].max().max()) + 1000000, (0, int(ventas[columnas_meses].max().max()) + 1000000))
            ...
            df_segmento_final = df_con_promedio[(df_con_promedio['PROMEDIO_MENSUAL_12'] >= segmento_promedio[0]) & (df_con_promedio['PROMEDIO_MENSUAL_12'] <= segmento_promedio[1])].copy()
